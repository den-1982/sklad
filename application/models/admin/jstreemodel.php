...
*	ADMIN
*/
class jstreeModel extends CI_Model
{
	...
	public function set_settings(array $options = array())
	{
		$this->options = array_merge($this->default, $options);
	}
	...
	public function mk($parent, $position = 0, $data = array()) 
	{
		...
		$sql[] = "INSERT INTO ".$this->options['structure_table']." (".implode(",", $this->options['structure']).") 
					VALUES (
						?".str_repeat(',?', count($this->options['structure']) - 1)."
					)";
		$tmp = array();
	...
	public function cp($id, $parent, $position = 0) 
	{
		...
		if (count($fields)) {
			try {
				$this->db->query("
						...
						SELECT ".$iid.",".implode(",",$fields)." 
							FROM ".$this->options['data_table']." 
							WHERE ".$this->options['data2structure']." = ".$id[$this->options['data2structure']]."
		...
		$new_nodes = $this->db->query("
			SELECT * FROM ".$this->options['structure_table']."
			WHERE ".$this->options['structure']["left"]." > ".$ref_lft." 
				AND ".$this->options['structure']["right"]." < ".($ref_lft + $width - 1)." 
				AND ".$this->options['structure']["id"]." != ".$iid."
			ORDER BY ".$this->options['structure']["left"]."
		")->result();
		...
		foreach ($new_nodes as $k => $node) {
			...
			if (count($fields)) {
				...
				$sql[] = "
					INSERT INTO ".$this->options['data_table']." (".$this->options['data2structure'].",".implode(",",$fields).")
					SELECT ".(int)$node[$this->options['structure']["id"]].",".implode(",",$fields)." 
						FROM ".$this->options['data_table']."
	...
	public function rm($id) 
	{
		...
		$sql[] = "
			...
			WHERE ".$this->options['structure']["left"]." >= ".(int)$lft." 
				AND ".$this->options['structure']["right"]." <= ".(int)$rgt."
		";
	...
	public function rn($id, $data) 
	{
		$res = (int)$this->db->query('
			SELECT 1 AS res 
			FROM '.$this->options['structure_table'].' 
			WHERE '.$this->options['structure']['id'].' = '.(int)$id
		)->row_array();
	...
	# проверка на ошибки структуры таблиц
	public function analyze($get_errors = false) 
	{
		$report = array();
		
		$res = (int)$this->db->query("SELECT COUNT(".$this->options['structure']["id"].") AS res FROM ".$this->options['structure_table']." WHERE ".$this->options['structure']["parent_id"]." = 0")->row()->res;
		if ($res !== 1) {
			$report[] = "No or more than one root node.";
		}
		
		$res = (int)$this->db->query("SELECT ".$this->options['structure']["left"]." AS res FROM ".$this->options['structure_table']." WHERE ".$this->options['structure']["parent_id"]." = 0")->row()->res;
		if ($res !== 1) {
		...
		$res = (int)$this->db->query("
			...
			WHERE
				...
				(SELECT COUNT(".$this->options['structure']['id'].") FROM ".$this->options['structure_table']." WHERE ".$this->options['structure']["id"]." = s.".$this->options['structure']["parent_id"].") = 0")->row()->res;
		if ($res > 0) {
			$report[] = "Missing parents.";
		}
		
		$res = (int)$this->db->query("SELECT MAX(".$this->options['structure']["right"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		$res2 = (int)$this->db->query("SELECT COUNT(".$this->options['structure']["id"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		if ($res / 2 !=	$res2){
			$report[] = "Right index does not match node count.";
		}
		
		$res = (int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']["right"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		$res2 = (int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']["left"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		if ($res != $res2) {
			$report[] = "Duplicates in nested set.";
		}
		
		$res = (int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']["id"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		$res2 = (int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']["left"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		if ($res != $res2) {
		...
		$res = (int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']["id"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		$res2 = (int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']["right"].") AS res FROM ".$this->options['structure_table'])->row()->res;
		if ($res != $res2) {
		...
		$res = (int)$this->db->query("
			...
			")->row()->res;
		if ($this->options['data_table'] && $res) {
	...
	public function reconstruct($analyze = true) 
	{
		...
		$counter = 2;
		if ( ! $this->db->query("SELECT COUNT(*) FROM temp_tree")->row()) {
			return false;
		}
		// $this->db->nextr();
